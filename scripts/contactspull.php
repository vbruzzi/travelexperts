...
//  get information about agencies from database.Create assoc. array in which each row is one agency.
    $resultagency1  = $dbh->query("SELECT AgencyId, AgncyCity, AgncyAddress,AgncyPhone, AgncyPostal,AgncyFax, AgncyProv, AgncyCountry from agencies ORDER BY AgncyCity asc")->fetch_all(MYSQLI_ASSOC);
...
// Select only agents with agencyID same as current row=agency=item, so that we select agents of one agency only.
        $query = "SELECT FirstName,LastName, Email,Phone FROM users INNER JOIN agents on users.UserId=agents.UserId 
        WHERE role=2 AND AgencyId= ".$item['AgencyId']." Order by FirstName";
       // echo $query;
         $resultagent1  = $dbh->query($query)->fetch_all(MYSQLI_BOTH); // get agents for each agency in big array
